...
def detect_polymarket_market():
    """5min markets: :00, :05, :10, :15, :20, :25, :30, :35, :40, :45, :50, :55"""
    now = datetime.now(timezone.utc)
    minute_block = (now.minute // 5) * 5  # Changed from 15 to 5
    market_start = now.replace(minute=minute_block, second=0, microsecond=0)
    market_end = market_start + timedelta(minutes=5)  # Changed from 15 to 5
    
    if now >= market_end:
        market_start += timedelta(minutes=5)  # Changed from 15 to 5
        market_end += timedelta(minutes=5)
    
    minutes_into = max(0, (now - market_start).total_seconds() / 60)
    time_left = max(0, (market_end - now).total_seconds() / 60)
    is_new_market = minutes_into < (5/60)  # First 5 seconds
...
# === MAIN DASHBOARD ===
st.title("🎯 **POLYMARKET 5-MINUTE TRACKER**")
st.markdown("**🔥 5min markets: :00, :05, :10... | Pyth + Chainlink + Binance sync**")
...
# === SIGNAL LOGIC (Adjusted for 5min) ===
if minutes_into <= 3 and abs(momentum) >= 0.15:  # Changed from 8 to 3
    confidence = 78 + min(10, abs(momentum) * 40)
    direction = "UP" if momentum > 0 else "DOWN"
elif minutes_into <= 4:  # Changed from 12 to 4
    direction = "UP" if momentum > 0 else "DOWN"
    confidence = 68
else:
    direction, confidence = "HOLD", 50

# **CHECK FOR MARKET END - RECORD WIN/LOSS (Adjusted for 5min)**
if minutes_into > 4.5 and len(st.session_state.trades) > 0:  # Changed from 14.5 to 4.5
...
st.progress(time_left / 5)  # Changed from 15 to 5
...
fig = go.Figure()
times = pd.date_range(end=datetime.now(), periods=60, freq='5s')  # Changed to 5s
...
fig.update_layout(height=500, title="5-Minute Momentum Tracking")
st.plotly_chart(fig, width='stretch')

# === SIDEBAR ===
st.sidebar.markdown("---")
st.sidebar.success("**🎯 5-MINUTE MARKETS:**")
st.sidebar.markdown("""
